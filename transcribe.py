...
def transcribe_audio(
    ...
    try:
        ...
        if progress_callback:
            progress_callback(0.70, "Validating audio file...")
        ...
        if progress_callback:
            progress_callback(0.72, "Starting transcription (this may take several minutes)...")
        
        # Transcribe with Whisper - pass the pre-loaded audio array
        device = get_device()
        try:
            ...
            if "'NoneType'" in str(ae):
                raise RuntimeError(f"Audio processing failed - the file may be corrupted or in an unsupported format. Original error: {ae}")
            raise
        except Exception as transcribe_err:
            raise RuntimeError(f"Whisper transcription failed: {transcribe_err}")
